...
from utils import (
    ...
    handle_argv,
    IMUDataset,
    load_bert_classifier_data_config,
    norm_processor,
...
def bert_classify(
    args, label_index, training_rate, label_rate, frozen_bert=False, balance=True
):
    data, labels, train_cfg, model_bert_cfg, model_classifier_cfg, dataset_cfg = (
        load_bert_classifier_data_config(args)
    )
    ...
    pipeline = [norm_processor(model_bert_cfg.feature_num)]
    ...
    classifier = fetch_classifier(
        method, model_classifier_cfg, input=model_bert_cfg.hidden, output=label_num
    )
    model = BERTClassifier(
        model_bert_cfg, classifier=classifier, frozen_bert=frozen_bert
    )
...
if __name__ == "__main__":
    ...
    method = "base_gru"
    args = handle_argv(
        "bert_classifier_" + method, "bert_classifier_train.json", method
    )
    if args.label_index != -1:
        label_index = args.label_index
    label_test, label_estimate_test = bert_classify(
        args,
        args.label_index,
        train_rate,
        label_rate,
