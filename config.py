...
def create_io_config(
    ...
    if "distill" in target:
        target = target.replace("distill", "pretrain")
    ...
    if pretrain_model is not None:
        if target.count("_") > 2:  # bert_classifier
            ...
            model_path = os.path.join(pretrain_model_path, pretrain_model)
...
def load_model_config(target, prefix, version):
    path_bert = "config/limu_bert.json"  # 预训练模型配置
    path_classifier = "config/classifier.json"  # 分类器配置

    if "bert" not in target:  # pretrain or pure classifier
        if "pretrain" in target or "distill" in target:
