...
DATASET_PATH = r"/home/taylor/DataSet/HHAR"
...
def extract_sensor(data, time_index, time_tag, window_time):
    ...
    while (
        index < len(data)
        and abs(data.iloc[index]["Creation_Time"] - time_tag) < window_time
    ):
        index += 1
    if index == time_index:
        return None, index
...
def preprocess_hhar(path, path_save, version, window_time=50, seq_len=40, jump=0):
    ...
    while time_index[0] < len(accs) and time_index[1] < len(gyros):
        print(f"progress: {time_index[0]}/{len(accs)}")
        acc, time_index_new_acc = extract_sensor(
            accs, time_index[0], time_tag, window_time=window_time * pow(10, 6)
        )
        gyro, time_index_new_gyro = extract_sensor(
            gyros, time_index[1], time_tag, window_time=window_time * pow(10, 6)
        )
        ...
        if acc is not None and gyro is not None and np.all(acc[-3:] == gyro[-3:]):
            ...
            if time_index[0] < len(accs) and time_index[1] < len(gyros):
                time_tag = min(
                    accs.iloc[time_index[0], 2], gyros.iloc[time_index[1], 2]
                )
            else:
                break
...
data, label = preprocess_hhar(
    DATASET_PATH, path_save, version, window_time=50, seq_len=20
)
