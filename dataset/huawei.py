...
def read_data(file_path):
    ...
    data = pd.read_csv(
        file_path, header=None, names=column_names, sep=" ", usecols=use_cols
    )
...
def read_and_save_data(path):
    ...
    for d, date in enumerate(date_list):
        for p, phone in enumerate(phone_list):
            data_path = (
                "D:/Dataset_Mobility/Huawei/SHLDataset_preview_v1/"
                + date
                + phone
                + "Motion.txt"
            )
            label_path = (
                "D:/Dataset_Mobility/Huawei/SHLDataset_preview_v1/" + date + "Label.txt"
            )
            ...
            total_data_temp["Position_label"] = pd.Series(
                np.ones(len(total_data_temp["Coarse_label"])) * p,
                index=total_data_temp.index,
            )
            total_data_temp["User_label"] = pd.Series(
                np.ones(len(total_data_temp["Coarse_label"])) * (d // user_num),
                index=total_data_temp.index,
            )
            ...
            total_data_temp = total_data_temp[
                (total_data_temp["Coarse_label"] == 1.0)
                | (total_data_temp["Coarse_label"] == 2.0)
                | (total_data_temp["Coarse_label"] == 3.0)
                | (total_data_temp["Coarse_label"] == 4.0)
            ]
    ...
    for p in range(positions.size):
        for u in range(users.size):
            ...
            for i in range(0, slice.shape[0] - 1, jump):
                ...
                if (
                    i + seq_len - 1 < slice.shape[0]
                    and tags[i + seq_len - 1] - tags[i] == seq_len - 1
                ):
                    labels = (slice.iloc[i : i + seq_len])[
                        ["Coarse_label", "Position_label", "User_label"]
                    ].values
                    if np.unique(labels[:, 0]).size == 1:
                        ...
                        if process_func:
                            data_new.append(
                                process_func(
                                    slice.iloc[i : i + seq_len][columns].values
                                )
                            )
                        else:
