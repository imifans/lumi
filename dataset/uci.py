...
DATASET_PATH = r"/home/taylor/DataSet/uci/RawData"
...
def preprocess(path, path_save, version, raw_sr=50, target_sr=20, seq_len=20):
    ...
    for root, dirs, files in source:
        print(f"progress {step_num}/{len(root)}")
        step_num = step_num + 1
        name_num = 0
        for name in files:
            print(f"progress {name_num}/{len(files)}")
            name_num = name_num + 1
            if name.startswith("acc"):
                ...
                for i in range(label_stat.shape[0]):
                    print(f"progress {i}/{len(root)}")
                    index_start = label_stat[i, 3]
                    index_end = label_stat[i, 4]
                    if "Identifier" in name:
                        continue
                    exp_data_acc = (
                        np.loadtxt(os.path.join(root, name), delimiter=" ") * 9.80665
                    )
                    exp_data_gyro = np.loadtxt(
                        os.path.join(root, "gyro" + name[3:]), delimiter=" "
                    )
                    exp_data = down_sample(
                        np.concatenate([exp_data_acc, exp_data_gyro], 1),
                        window_sample,
                        index_start,
                        index_end,
                    )
                    if exp_data.shape[0] > seq_len and label_stat[i, 2] <= 6:
                        ...
                        exp_data = exp_data.reshape(
                            exp_data.shape[0] // seq_len, seq_len, exp_data.shape[1]
                        )
                        exp_label = np.ones((exp_data.shape[0], exp_data.shape[1], 1))
                        exp_label = np.concatenate(
                            [
                                exp_label * label_stat[i, 2],
                                exp_label * label_stat[i, 1],
                            ],
                            2,
                        )
...
path_save = r"uci"
version = r"20_20"
data, label = preprocess(DATASET_PATH, path_save, version, target_sr=20, seq_len=20)
