...
DATASET_PATH = r"/home/taylor/DataSet/motion"
                ...
                for name in files_exp:
                    ...
                    if sensor_down.shape[0] > seq_len:
                        sensor_down = sensor_down[
                            : sensor_down.shape[0] // seq_len * seq_len, :
                        ]
                        sensor_down = sensor_down.reshape(
                            sensor_down.shape[0] // seq_len,
                            seq_len,
                            sensor_down.shape[1],
                        )
                        sensor_label = np.ones(
                            (sensor_down.shape[0], sensor_down.shape[1], 1)
                        )
...
def preprocess(path, path_save, version, target_window=50, seq_len=20):
    data_acc, label_acc = load_sensor_data(
        os.path.join(path, "Accelerometer"), seq_len, target_window
    )
    data_gyro, label_gyro = load_sensor_data(
        os.path.join(path, "Gyroscope"), seq_len, target_window
    )
    data = []
    label = []
    for i in range(len(data_acc)):
        print(f"processeing. {i}/{len(data_acc)}")
        len_min = min(data_acc[i].shape[0], data_gyro[i].shape[0])
        data.append(
            np.concatenate([data_acc[i][:len_min] * 9.8, data_gyro[i][:len_min]], 2)
        )
...
path_save = r"motion"
version = r"20_20"
data, label = preprocess(DATASET_PATH, path_save, version, seq_len=20)
