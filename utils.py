...
def prepare_pretrain_dataset(data, labels, training_rate, seed=None):
    ...
    print(
        f"data_train shape = {data_train.shape}, data_vali shape = {data_vali.shape}, data_test shape = {data_test.shape}"
    )
...
def prepare_classifier_dataset(
    ...
    if balance:
        (
            ...
            data_test_labeled,
            label_test_labeled,
        ) = prepare_simple_dataset_balance(
            data_train, label_train, training_rate=label_rate
        )
    else:
        (
            data_train_labeled,
...
def partition_and_reshape(
    ...
    if change_shape and merge != 0:
        data_train, label_train = merge_dataset(
            data_train, label_train, mode=merge_mode
        )
    ...
    print(
        "Train Size: %d, Vali Size: %d, Test Size: %d"
...
class buildDataSet4PreTrain(Dataset):
    """Load sentence pair (sequential or random order) from corpus"""
...
def load_pretrain_data_config(args):
    ...
    training_rate = train_cfg.training_rate
    ...
    labels = np.load(args.label_path).astype(np.float32)
    print(
        f"load data success, data.shape = {data.shape},  labels.shape = {labels.shape}"
    )

    return data, labels, train_cfg, model_cfg, mask_cfg, dataset_cfg, training_rate
